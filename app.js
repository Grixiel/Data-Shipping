...
// --- BASE DE DADOS DE ROTAS (CINTO DE SEGURANÇA IMEDIATO) ---
...
function carregarMapaImediato() {
    ...
    CURRENT_R_STR.split(',').forEach(g => {
        let [t, r] = g.split(':');
        if(r) { r.split('|').forEach(x => { ROUTE_LIST.push({ t: parseInt(t), r: x }); RAMPA_MAP[parseInt(t)].push(x); }); }
    });
}

window.onload = () => {
    carregarMapaImediato(); // Carrega as rotas instantaneamente!
    carregarMapaTobogas(); // Atualiza em segundo plano pelo Sheets
...
function carregarMapaTobogas() {
    ...
    }).catch(e => console.error("Erro Planilha (Usando rotas estáticas)", e));
}
