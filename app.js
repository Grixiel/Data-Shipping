...
window.onerror = function(msg, url, line) {
    fecharLoader();
    setStatusUi('Erro de Sintaxe no Código (Verifique as aspas da URL)', 'bg-rose-600');
    console.error("Erro fatal:", msg, "na linha:", line);
};
...
// --- INICIALIZAÇÃO ---
window.onload = () => { 
    fecharLoader(); // Garante que a tela inicia libertada
...
function buscarDadosDaNuvem() {
    ...
    script.onerror = function() {
        setStatusUi('Erro: Conexão bloqueada com o Google', 'bg-rose-500');
    };
...
function carregarDadosLocais() {
    try {
        ...
        console.error("Cache local corrompido. Formatando memória...");
        localStorage.removeItem("WMS_DATA_SHIPPING_V2");
        DATA_CACHE = null;
        AVAILABLE_HOURS = [];
    }
...
function processarTextoCola(veioDaNuvem = false) {
    ...
    setTimeout(() => {
        try {
            ...
            if(arr.length === 0) { 
                if(!veioDaNuvem) { alert("Nenhum dado legível encontrado no texto."); fecharLoader(); }
                else { setStatusUi('Dados da Nuvem incompreensíveis', 'bg-rose-500'); }
                return; 
            }
            ...
            console.error(e); 
            if(!veioDaNuvem) { alert("Erro ao calcular dados: " + e.message); fecharLoader(); }
            else { setStatusUi('Erro no cálculo da Nuvem', 'bg-rose-500'); }
        }
    }, 500);
}
